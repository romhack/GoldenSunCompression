...
void checkSrcBit()
{
	...
	if (C == 0) printf ("SrcBit is 0\n");
	else printf ("SrcBit is 1\n");
...
void checkIterBit()
{
	...
	if (C == 0) printf ("IterBit is 0\n");
	else printf ("IterBit is 1\n");
}
...
int main(int argc, char *argv[])
{
	...
	count = 0;                                        
	SetInitials(0, &glast, &gsrc, &gflags);           
...
u16 GetNextChararacter(u16 last, u32 * src, u32 flags)
{
	...
	if (C == 0) {
		for (;;) 
		{	//printf ("iter bit is zero\n");
	
			checkSrcBit();
			if (C ==1) {
				//printf ("source bit is one\n");
			...
			//else printf ("source bit is zero\n");
	...
	//else printf ("iterbit is 1 instantly\n");	
